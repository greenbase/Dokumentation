...
\begin{table}[ht]
	\centering
	\begin{tabularx}{\hsize}{|c|l|c|}
		\hline
		\rowcolor{lightgray}
		Kriterium & Wertezuordnung & Gewichtung\\ 
		\hline
		\makecell{Baumtiefe\\(Komplexität)} & \makecell[l]{$\begin{array}{lcl}\num{1} &\rightarrow& \num{1}\\\num{2} &\rightarrow& \num{0,75}\\\num{3} &\rightarrow& \num{0,5}\\\num{4} &\rightarrow& \num{0,25}\\>=\num{5} &\rightarrow& \num{0}\end{array}$}& \num{0.165}\\
		\hline
		\makecell{Baumanzahl\\(Komplexität)} & \makecell[l]{$\begin{array}{lcl}x<=\num{19} &\rightarrow& \num{1}\\\num{20}<=x<=\num{39} &\rightarrow& \num{0,75}\\\num{40}<=x<=\num{59} &\rightarrow& \num{0,5}\\\num{60}<=x<=\num{79} &\rightarrow& \num{0,25}\\x>=\num{80} &\rightarrow& \num{0}\end{array}$} & \makecell{\num{0.055}}\\
		\hline
		\makecell{Sensitivität}& \makecell[l]{$\begin{array}{lcl}\SI{95}{\percent}<=x<=\SI{100}{\percent} &\rightarrow& \num{1}\\\SI{90}{\percent}<=x<\SI{95}{\percent} &\rightarrow& \num{0,75}\\\SI{85}{\percent}<=x<\SI{90}{\percent} &\rightarrow& \num{0,5}\\\SI{80}{\percent}<=x<\SI{85}{\percent} &\rightarrow& \num{0,25}\\<x<\SI{80}{\percent} &\rightarrow& \num{0}\end{array}$} & \makecell{\num{0.44}}\\
		\hline
		\makecell{Relevanz} & \makecell[l]{$\begin{array}{lcl}\SI{75}{\percent}<=x<\SI{100}{\percent} &\rightarrow& \num{1}\\\SI{50}{\percent}<=x<\SI{75}{\percent} &\rightarrow& \num{0,5}\\\SI{25}{\percent}<=x<\SI{50}{\percent} &\rightarrow& \num{0,25}\\x<\SI{25}{\percent} &\rightarrow& \num{0}\end{array}$} & \makecell{\num{0.33}}\\
		\hline
