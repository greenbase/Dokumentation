...
\begin{table}[ht]
	\centering
	\begin{tabularx}{\hsize}{|c|r|r|c|}
		\hline
		\rowcolor{lightgray}
		Kriterium & Wertebereiche & \makecell[cc]{Kriteriums-\\bewertung} & Gewichtung\\ 
		\hline
		...
		& $x<\SI{80}{\percent}$ & \num{0}&\\\hline
		\multirowcell{4}{Relevanz} & $\SI{75}{\percent}<=x<\SI{100}{\percent}$ & \num{1}  & \multirowcell{4}{\num{0.33}}\\
		\cline{2-3}
		& $\SI{50}{\percent}<=x<\SI{75}{\percent}$ & \num{0,75}&\\\cline{2-3}
		& $\SI{25}{\percent}<=x<\SI{50}{\percent}$&\num{0,5}\\\cline{2-3}
		& $x<\SI{25}{\percent}$&\num{0}\\\hline
